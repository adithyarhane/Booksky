...
const BookListPaginationSec = () => {
  const { booksData, page, handleFilterUrl } = useBookContext();
  return (
    <div>
      <div className="mt-24 flex flex-col items-center border-t border-stone-100 pt-16">
        <div className="flex items-center justify-between w-full max-w-2xl px-4">
            ...
            <div className="flex items-center gap-2 h-6">
              {[...Array(booksData.totalPages)].map((_, i) => (
                ...
                >
                  ...
                  />
                  {/* Tooltip on hover */}
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-[8px] font-black opacity-0 group-hover:opacity-100 transition-opacity">
                    {i + 1}
                  </span>
                </button>
              ))}
          ...
          {/* Right: Refined Next */}
          <button
            disabled={Number(page) === Number(booksData.totalPages)}
            onClick={() => {
              handleFilterUrl("page", Number(page) + 1);
              window.scrollTo({ top: 0, behavior: "smooth" });
            }}
...
export default BookListPaginationSec;
