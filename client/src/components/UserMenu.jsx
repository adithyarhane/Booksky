...
const UserMenu = () => {
  const { userData, logout, sendVerificationOTP } = useAuthContext();

  return (
    <div className="relative group">
      {/* --- THE TRIGGER BUTTON --- */}
      <button className="flex items-center gap-2.5 p-1.5 pr-4 bg-[#3E2723] border border-white/10 rounded-full text-white shadow-lg hover:bg-[#4E342E] transition-all duration-300">
        <div className="w-8 h-8 bg-[#D7CCC8] rounded-full flex items-center justify-center text-[#3E2723] font-bold text-sm shadow-inner">
          {userData ? userData.name[0].toUpperCase() : <FiUser size={16} />}
        </div>
        ...
        <FiChevronDown
          size={14}
          className="text-[#A1887F] group-hover:rotate-180 transition-transform duration-500"
        />
      </button>
